...
type FeedbackTrendsCopy = {
  title: string; range: string; evaluation: string; feedbackVolume: string;
...
export function FeedbackTrends({ locale }: { locale: Locale }) {
  const t = localeMessages<FeedbackTrendsCopy>(locale, "dashboardFeedbackTrends"),
    hints = localeMessages<Record<string, string>>(locale, "chartHints");
  ...
  return <section className="panel improvement-trends dashboard-feedback-trends">
    <div className="trend-head"><h2><SectionInfo title={t.title} description={hints[t.title]} /></h2><label>{t.range}<select value={hours} onChange={(event) => setHours(Number(event.target.value))}><option value={24}>24h</option><option value={72}>3d</option><option value={168}>7d</option><option value={720}>30d</option></select></label></div>
    <div className="analytics-grid">
      ...
      <Card title={t.activeTrend} locale={locale}><ResponsiveContainer width="100%" height={250}><AreaChart data={data?.active_evaluations ?? []} margin={{ left: -18 }}><CartesianGrid vertical={false} /><XAxis dataKey="time" tickFormatter={tick} /><YAxis allowDecimals={false} /><Tooltip labelFormatter={(value) => new Date(String(value)).toLocaleString(intlLocales[locale])} /><Area type="monotone" dataKey="count" name={t.activeCount} stroke="var(--accent)" fill="var(--surface-raised)" dot={{ r: 3, fill: "var(--accent)", stroke: "var(--surface)" }} /></AreaChart></ResponsiveContainer></Card>
    </div>
    <Card title={t.iterationTrend} locale={locale}><div className="chart-select"><label>{t.evaluation}<select value={build} onChange={(event) => setBuild(event.target.value)}>{data?.iteration_trends.map((item) => <option key={item.build_id} value={item.build_id}>{item.name}</option>)}</select></label></div>{trend?.points.length ? <ResponsiveContainer width="100%" height={280}><ComposedChart data={trend.points} margin={{ left: -18 }}><CartesianGrid vertical={false} /><XAxis dataKey="iteration" tickFormatter={(value) => `#${value}`} /><YAxis yAxisId="count" allowDecimals={false} /><YAxis yAxisId="score" orientation="right" domain={[0, 10]} /><Tooltip /><Legend /><Bar yAxisId="count" dataKey="accepted_count" name={t.accepted} fill="#79c99e" /><Line yAxisId="score" type="monotone" dataKey="score" name={t.score} stroke="#8fb8ff" strokeWidth={3} /></ComposedChart></ResponsiveContainer> : <p className="hint">{t.noIterationFeedback}</p>}</Card>
    <div className="analytics-grid">
      <Card title={t.feedbackStatus} locale={locale}><ResponsiveContainer width="100%" height={250}><BarChart data={data?.feedback_status ?? []} margin={{ left: -18 }}><CartesianGrid vertical={false} /><XAxis dataKey="name" tickFormatter={short} /><YAxis allowDecimals={false} /><Tooltip /><Legend /><Bar stackId="a" dataKey="proposed" name={t.proposed} fill="#f1d292" /><Bar stackId="a" dataKey="adopted" name={t.adopted} fill="#79c99e" /><Bar stackId="a" dataKey="rejected" name={t.rejected} fill="#eaa89f" /></BarChart></ResponsiveContainer></Card>
      <Card title={t.issueSeverity} locale={locale}><ResponsiveContainer width="100%" height={250}><AreaChart data={data?.issue_severity ?? []} margin={{ left: -18 }}><XAxis dataKey="time" tickFormatter={tick} /><YAxis allowDecimals={false} /><Tooltip labelFormatter={(value) => new Date(String(value)).toLocaleString(intlLocales[locale])} /><Legend /><Area stackId="a" type="monotone" dataKey="low" name={t.low} fill="#79c99e" stroke="#79c99e" /><Area stackId="a" type="monotone" dataKey="medium" name={t.medium} fill="#f1d292" stroke="#f1d292" /><Area stackId="a" type="monotone" dataKey="high" name={t.high} fill="#e49a64" stroke="#e49a64" /><Area stackId="a" type="monotone" dataKey="critical" name={t.critical} fill="#eaa89f" stroke="#eaa89f" /></AreaChart></ResponsiveContainer></Card>
    </div>
