...
type UsageEvent = { id: string; time: string; model: string; status: number; failed: boolean; input_tokens: number; output_tokens: number; total_tokens: number; input_chars: number; output_chars: number; tool_count: number; duration_ms: number; error: string; limit_requests?: string; remaining_requests?: string; limit_tokens?: string; remaining_tokens?: string; retry_after?: string };
type AiUsage = { minutes: number; summary: { requests: number; tokens: number; errors: number; success_rate: number; average_duration_ms: number; rpm: number; tpm: number }; limits: Record<string, string | undefined>; timeline: { time: string; requests: number; tokens: number }[]; events: UsageEvent[] };
type Copy = { title: string; description: string; range: string; refresh: string; rpm: string; tpm: string; successRate: string; averageLatency: string; requests: string; tokens: string; limits: string; remainingRequests: string; remainingTokens: string; retryAfter: string; requestTrend: string; tokenTrend: string; recentCalls: string; time: string; model: string; status: string; usage: string; latency: string; details: string; input: string; output: string; tools: string; empty: string; unavailable: string; noLimitData: string };
  ...
  return <section className="panel app-settings ai-usage-monitor">
    ...
    {!data ? <p className="hint section-description">{t.unavailable}</p> : <>
      <div className="ai-usage-metrics">
        <Metric label={t.rpm} value={format(data.summary.rpm)} /><Metric label={t.tpm} value={format(data.summary.tpm)} />
      ...
      <div className="analytics-grid">
        <article className="analytics-chart"><h3>{t.requestTrend}</h3><ResponsiveContainer width="100%" height={190}><AreaChart data={data.timeline} margin={{ left: -18 }}><CartesianGrid vertical={false} /><XAxis dataKey="time" tickFormatter={(value) => tick(String(value), locale)} /><YAxis allowDecimals={false} /><Tooltip labelFormatter={(value) => new Date(String(value)).toLocaleString(intlLocales[locale])} /><Area type="monotone" dataKey="requests" name={t.requests} stroke="var(--accent)" fill="var(--surface-raised)" /></AreaChart></ResponsiveContainer></article>
        <article className="analytics-chart"><h3>{t.tokenTrend}</h3><ResponsiveContainer width="100%" height={190}><AreaChart data={data.timeline} margin={{ left: -18 }}><CartesianGrid vertical={false} /><XAxis dataKey="time" tickFormatter={(value) => tick(String(value), locale)} /><YAxis allowDecimals={false} /><Tooltip labelFormatter={(value) => new Date(String(value)).toLocaleString(intlLocales[locale])} /><Area type="monotone" dataKey="tokens" name={t.tokens} stroke="#79c99e" fill="#79c99e33" /></AreaChart></ResponsiveContainer></article>
      </div>
      <div className="ai-usage-limits"><strong>{t.limits}</strong>{limits?.remaining_requests || limits?.remaining_tokens ? <span>{t.remainingRequests}: {limits.remaining_requests ?? "—"}{limits.limit_requests ? ` / ${limits.limit_requests}` : ""} · {t.remainingTokens}: {limits.remaining_tokens ?? "—"}{limits.limit_tokens ? ` / ${limits.limit_tokens}` : ""}{limits.retry_after ? ` · ${t.retryAfter}: ${limits.retry_after}s` : ""}</span> : <span className="hint">{t.noLimitData}</span>}</div>
      <div className="ai-usage-table"><h3>{t.recentCalls}</h3><DataTable columns={columns} rows={data.events} empty={t.empty} onRowClick={setSelected} gridTemplateColumns="minmax(100px,.8fr) minmax(130px,1.2fr) 76px minmax(120px,1fr) 100px" /></div>
      {selected && <div className="ai-usage-detail"><strong>{t.details}</strong><span>{t.input}: {format(selected.input_tokens)} tokens / {format(selected.input_chars)} chars · {t.output}: {format(selected.output_tokens)} tokens / {format(selected.output_chars)} chars · {t.tools}: {selected.tool_count}</span>{selected.error && <span className="ai-usage-error">{selected.error}</span>}</div>}
    </>}
  </section>;
}

function Metric({ label, value, warning = false }: { label: string; value: string; warning?: boolean }) { return <div><span>{label}</span><strong className={warning ? "ai-usage-error" : ""}>{value}</strong></div>; }
