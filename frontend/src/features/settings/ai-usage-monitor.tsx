...
export function AiUsageMonitor({ locale }: { locale: Locale }) {
  ...
  return <section className="panel app-settings ai-usage-monitor">
    ...
    {!data ? <p className="hint section-description">{t.unavailable}</p> : <>
      ...
      <div className="ai-usage-table"><h3>{t.recentCalls}</h3><div className="ai-usage-table-scroll"><DataTable columns={columns} rows={data.events} empty={t.empty} onRowClick={setSelected} gridTemplateColumns="minmax(100px,.8fr) minmax(130px,1.2fr) 76px minmax(120px,1fr) 100px" /></div></div>
