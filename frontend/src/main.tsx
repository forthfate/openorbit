...
export default function App(){
  ...
  const save=()=>{if(!room.settingsTested){room.setNotice('Test the configuration before saving.');return}api('/api/settings','PUT',room.settings).then(()=>{room.setSettingsTested(false);room.setNotice('AI settings saved');room.refresh()})}
  ...
  const content={dashboard:<DashboardPage data={room.data} logs={room.orbitLogs} locale={locale} onOpenRun={()=>setPage('runs')} onOpenBuild={()=>setPage('builds')}/>,assets:<AssetsPage locale={locale} profiles={room.profiles} workflows={room.workflows} runners={room.runners} settings={room.settings} setSettings={room.setSettings} save={save} test={test} tested={room.settingsTested} onCreateWorkflow={createWorkflow} onUpdateWorkflow={updateWorkflow} onDelete={deleteAsset}/>,builds:<EvaluationBuildsPage locale={locale} builds={room.builds} workflows={room.workflows} runs={room.runs} profiles={room.profiles} onInvoke={invoke} onTest={testBuild} onCreate={createBuild} onUpdate={updateBuild} onDelete={deleteBuild} onCreateWorkflow={createWorkflow}/>,runs:<EvaluationsPage locale={locale} runs={room.activeRuns} onStop={stopRun} onEmergencyStop={()=>setConfirmingEmergencyStop(true)}/>,improvements:<ImprovementsPage items={room.improvements}/>,settings:<SettingsPage locale={locale} setLocale={setLocale} theme={theme} setTheme={setTheme}/>}[page]
