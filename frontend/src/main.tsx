...
export default function App(){
  ...
  const test=()=>api<{response:string}>('/api/settings/hello','POST',room.settings).then(r=>{room.setSettingsTested(true);room.setNotice(`Test succeeded: ${r.response}`)}).catch(e=>room.setNotice(e.message))
  const stop=()=>api('/api/runs/emergency-stop','POST').then(()=>{room.setNotice('All active runs stopped');room.refresh()})
  const confirmEmergencyStop=()=>{setConfirmingEmergencyStop(false);stop()}
  const stopRun=(id:string)=>api(`/api/runs/${id}/cancel`,'POST').then(()=>{room.setNotice('Evaluation stopped');room.refresh()}).catch(e=>room.setNotice(e.message))
  const deleteRuns=(ids:string[])=>Promise.all(ids.map(id=>api(`/api/runs/${encodeURIComponent(id)}`,'DELETE'))).then(()=>{room.setNotice(`${ids.length} evaluation run${ids.length===1?'':'s'} deleted`);return room.refresh()}).catch(e=>{room.setNotice(e.message);throw e})
  const approveRun=(id:string)=>api(`/api/runs/${id}/approve`,'POST').then(()=>{room.setNotice('Evaluation approved');room.refresh()}).catch(e=>room.setNotice(e.message))
  const rejectRun=(id:string)=>api(`/api/runs/${id}/reject`,'POST').then(()=>{room.setNotice('Evaluation rejected');room.refresh()}).catch(e=>room.setNotice(e.message))
  const save=()=>{if(!room.settingsTested){const error=new Error('Test the configuration before saving.');room.setNotice(error.message);return Promise.reject(error)}return api('/api/settings','PUT',room.settings).then(()=>{room.setSettingsTested(false);room.setNotice('AI settings saved');return room.refresh()})}
  const invoke=(id:string)=>api(`/api/evaluation-builds/${id}/runs`,'POST').then(()=>{room.setNotice('Evaluation started');room.refresh()}).catch(e=>room.setNotice(e.message))
  const testBuild=(id:string)=>api(`/api/evaluation-builds/${id}/tests`,'POST').then(()=>{room.setNotice('Evaluation test started');room.refresh()}).catch(e=>room.setNotice(e.message))
  const createBuild=(values:unknown)=>api('/api/evaluation-builds','POST',values).then(()=>{room.setNotice('Evaluation build created');room.refresh()}).catch(e=>room.setNotice(e.message))
  const updateBuild=(id:string,values:unknown)=>api(`/api/evaluation-builds/${id}`,'PUT',values).then(()=>{room.setNotice('Evaluation build updated');room.refresh()}).catch(e=>room.setNotice(e.message))
  const deleteAsset=(kind:'profile'|'template'|'test-set'|'runner'|'workflow',id:string)=>{const path={profile:`/api/settings/profiles/${encodeURIComponent(id)}`,template:`/api/prompt-templates/${id}`,'test-set':`/api/target-test-case-sets/${id}`,runner:`/api/runners/${id}`,workflow:`/api/workflows/${id}`}[kind];api(path,'DELETE').then(()=>{room.setNotice('Asset deleted');room.refresh()}).catch(e=>room.setNotice(e.message))}
  const deleteBuild=(id:string)=>setDeletingBuild(id)
  const confirmDeleteBuild=()=>{if(!deletingBuild)return;const id=deletingBuild;setDeletingBuild(null);api(`/api/evaluation-builds/${id}`,'DELETE').then(()=>{room.setNotice('Evaluation build deleted');room.refresh()}).catch(e=>room.setNotice(e.message))}
  ...
  const content={dashboard:<DashboardPage data={room.data} logs={room.orbitLogs} locale={locale} onOpenRun={()=>setPage('runs')} onOpenBuild={()=>setPage('builds')}/>,assets:<AssetsPage locale={locale} workflows={room.workflows} runners={room.runners} promptTemplates={room.promptTemplates} testCaseSets={room.testCaseSets} onRefresh={room.refresh} onCreateWorkflow={createWorkflow} onUpdateWorkflow={updateWorkflow} onDelete={deleteAsset}/>,builds:<EvaluationBuildsPage locale={locale} builds={room.builds} workflows={room.workflows} runs={room.runs} profiles={room.profiles} promptTemplates={room.promptTemplates} testCaseSets={room.testCaseSets} onInvoke={invoke} onTest={testBuild} onCreate={createBuild} onUpdate={updateBuild} onDelete={deleteBuild} onCreateWorkflow={createWorkflow}/>,runs:<EvaluationsPage locale={locale} runs={room.runs} onStop={stopRun} onApprove={approveRun} onReject={rejectRun} onEmergencyStop={()=>setConfirmingEmergencyStop(true)} onDeleteRuns={deleteRuns}/>,improvements:<ImprovementsPage/>,settings:<SettingsPage locale={locale} setLocale={setLocale} theme={theme} setTheme={setTheme} profiles={room.profiles} settings={room.settings} setSettings={room.setSettings} test={test} save={save} tested={room.settingsTested} onDeleteProfile={id=>deleteAsset('profile',id)}/>}[page]
