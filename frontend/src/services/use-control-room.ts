...
import type { Build, Dashboard, ExecutionEnvironment, OrbitLog, PromptTemplate, Run, RunnerAsset, Settings, SystemReadiness, TargetEnvironment, TargetTestCaseSet } from '../domain/models'
...
export function useControlRoom(){
  ...
  const refresh=useCallback(()=>Promise.all([api<Dashboard>('/api/dashboard'),api<Build[]>('/api/builds'),api<RunnerAsset[]>('/api/runners'),api<Run[]>('/api/runs'),api<OrbitLog[]>('/api/orbit-logs'),api<PromptTemplate[]>('/api/prompt-templates'),api<TargetTestCaseSet[]>('/api/target-test-case-sets'),api<ExecutionEnvironment[]>('/api/execution-environments'),api<TargetEnvironment[]>('/api/target-environments'),api<Settings>('/api/settings'),api<Settings[]>('/api/settings/profiles'),api<SystemReadiness>('/api/system/readiness')]).then(([d,b,rr,r,o,t,tc,ee,te,s,p,readiness])=>{setData(d);setBuilds(b);setRunners(rr);setRuns(r);setOrbitLogs(o);setPromptTemplates(t);setTestCaseSets(tc);setExecutionEnvironments(ee);setTargetEnvironments(te);setProfiles(p);setReadiness(readiness);if(!settingsLoaded.current){setSettingsState(s);settingsLoaded.current=true}}).catch(()=>setNotice(ui.apiConnectionFailed,'warning')).finally(()=>setLoading(false)),[setNotice,ui])
  useEffect(()=>{refresh();const timer=window.setInterval(refresh,3000);return()=>window.clearInterval(timer)},[refresh])
  return{data,builds,workflows:[],runners,runs,orbitLogs,promptTemplates,testCaseSets,executionEnvironments,targetEnvironments,profiles,settings,readiness,setSettings,settingsTested,setSettingsTested,loading,setNotice,refresh}
}
