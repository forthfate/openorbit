import { Activity, BarChart3, Boxes, FileCode2, PanelLeftClose, Play, Settings, Sparkles } from 'lucide-react'
...
export function AppShell({page,setPage,locale,theme,headerAction,activeRunCount=0,children}:{page:Page;setPage:(page:Page)=>void;locale:Locale;theme:string;headerAction?:ReactNode;activeRunCount?:number;children:ReactNode}){
  ...
  return <main className={collapsed?'sidebar-collapsed':undefined} data-theme={theme==='midnight'?'midnight':undefined}><aside><button className="sidebar-toggle" type="button" aria-label={sidebarLabel} aria-expanded={!collapsed} title={sidebarLabel} onClick={()=>setCollapsed(value=>!value)}><PanelLeftClose size={18}/></button>{collapsed?<button className="brand brand--expand" type="button" aria-label="Expand navigation" title="Expand navigation" onClick={()=>setCollapsed(false)}><Sparkles size={20}/></button>:<div className="brand"><Sparkles size={20}/><div><span>OpenOrbit</span><small>{__OPENORBIT_VERSION__}</small></div></div>}<nav>{navigation.map(([id,icon,label])=><button className={page===id?'active':''} onClick={()=>setPage(id)} key={id} aria-label={label} title={collapsed?label:undefined}>{icon}<span className="nav-label">{label}</span>{id==='runs'&&activeRunCount>0&&<span className="nav-run-count" aria-label={`${activeRunCount} active runs`}>{activeRunLabel}</span>}</button>)}</nav></aside><section className="content"><header><div className="page-header-copy"><h1>{t[page]}</h1><p className="sub">{pageDescriptions[page]}</p></div><div className="page-header-actions">{dashboardRepositoryLink}{headerAction}</div></header>{children}<footer><span>© insighta cloud Inc.</span><a className="github-link" href="https://github.com/forthfate/openorbit" target="_blank" rel="noreferrer" aria-label="OpenOrbit on GitHub" title="OpenOrbit GitHub repository"><SiGithub size={18}/></a></footer></section><ChatAssistant/></main>
}
