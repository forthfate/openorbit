...
test('opens the build and profile editors with editable lifecycle fields', async ({ page }) => {
  await page.goto(BASE_URL + '/#builds')
...
test('creates a safe task in the UI, then completes Test and Run', async ({ page }) => {
  ...
  await expect.poll(async () => (await page.request.get(`${BASE_URL}/api/runs`)).json().then((runs: { evaluation_build_id?: string; status: string }[]) => runs.find(run => run.evaluation_build_id === buildId && run.status === 'succeeded')?.status)).toBe('succeeded')
  await buildRow.getByRole('button', { name: 'Run' }).click()
  await expect.poll(async () => (await page.request.get(`${BASE_URL}/api/runs`)).json().then((runs: { evaluation_build_id?: string; execution_mode?: string; status: string }[]) => runs.find(run => run.evaluation_build_id === buildId && run.execution_mode === 'run' && run.status === 'succeeded')?.status)).toBe('succeeded')
})
