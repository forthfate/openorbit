...
def _aws_profile_credentials(profile: str) -> tuple[str, str, str]:
    ...
    credentials_path = Path(shared_credentials).expanduser() if shared_credentials else Path.home() / ".aws" / "credentials"
...
class BedrockProvider:
    def complete(self, settings: ModelSettings, prompt: str) -> str:
        ...
        if not access_key or not secret_key or not settings.model:
            raise RuntimeError("AWS Bedrock requires a valid AWS CLI profile or available AWS credentials, and a model.")
        ...
        def sign(key: bytes, value: str) -> bytes: return hmac.new(key, value.encode(), hashlib.sha256).digest()
        signing_key = sign(sign(sign(sign(("AWS4" + secret_key).encode(), date_stamp), settings.region), "bedrock"), "aws4_request")
